 ...
 * - 这个接口依赖的中间件的数组
 * - 访问路径
 * - 实现, 形式是 (req: Request, 参数: Record<string, any>) => Aff<Record<string, any>>
 * 实现中的'参数'字段是req.body
 * 最后接口会返回{err: ?, data: ?}格式
...
export type 接口_ED模式 = {
  ...
  [参数]: {
    中间件们: 中间件[]
    访问路径: string
    接口实现: (req: Request, 参数: any) => Aff<Record<string, any>>
  }
}
    ...
    接口实现: (req: Request, res: Response) =>
      Aff(async function () {
        ...
        try {
          ...
          var c = await 转为Promise(a[参数].接口实现(req, req.body))
          log(D, '%o', {
            行为: '调用结束',
            调用id,
            路径: req.path,
